...
- Scoped React capabilities (`eslint-plugin-react-hooks` and component naming, scoped internally to `**/*.{jsx,tsx}`)
...
### 3. File Scoping for Framework Rules

Framework-specific rules that require JSX or React AST semantics are restricted through internal ESLint `files` patterns:
...
Non-JSX files remain unaffected while JSX/TSX files receive full React Hooks enforcement without requiring a separate profile.

### 4. Consumer Tarball Verification

Update `scripts/verify-tarball/verify-tarball.mts` to:

- Test only the root `eslint-config-yarapa` import.
- Validate that the single default export lints JavaScript, TypeScript, and React JSX files simultaneously.
- Verify that violating code triggers expected diagnostic rules (`no-var`, `react-hooks/rules-of-hooks`).
...
- [x] The default config contains React/JSX and React Hooks behavior, file-scoped to `**/*.{jsx,tsx}`.
...
- [x] Relevant tests prove JavaScript, TypeScript, Node, React, and mixed/full-stack consumption through the root import.
